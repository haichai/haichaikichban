...
// --- FIREBASE CONFIG ---
// Firebase apiKey trong config client KHÔNG phải secret. Gemini API key mới là secret.
...
const INITIAL_SCRIPTS = [
  {
    ...
    scenes: [
      {
        ...
        content: 'Nhiều người nghĩ mở chuỗi thì cứ tìm ngã tư, mặt tiền rộng là thắng. Tôi cũng từng nghĩ thế.',
    ...
    caption: 'Bài học xương máu khi đi tìm nhà cho Haichai. #haichai #kinhdoanh #mochuoi',
...
const emitGeminiStatus = (message) => {
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('haichai-gemini-status', { detail: message })
    );
  }
};
...
// Tối ưu Retry: Fail Fast với lỗi Client, chỉ retry nếu dính 429 hoặc 500.
const callGeminiWithRetry = async (systemPrompt, userPrompt, schema, retries = 2) => {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      emitGeminiStatus(`Đang gọi Gemini ${GEMINI_MODEL}${i > 0 ? ` (Thử lại lần ${i})...` : '...'}`);
      ...
      emitGeminiStatus('Gemini đã phản hồi, đang xử lý dữ liệu...');
      const responseText = result.candidates?.[0]?.content?.parts?.[0]?.text;
      return extractJsonFromText(responseText);

    } catch (error) {
      console.error(`Lỗi gọi Gemini (Lần ${i + 1}):`, error);

      // KHÔNG RETRY nếu lỗi từ phía Client (Key sai, Model không có, Lỗi Prompt)
      if ([400, 401, 403, 404].includes(error.status)) {
        throw error;
      }

      // Hết lần thử thì bắn lỗi ra ngoài để giao diện báo đỏ
      if (i === retries - 1) throw error;

      // Nếu dính Rate Limit (429) hoặc Network Error, nghỉ 2s rồi thử lại
...
- duplicateRiskScore là điểm đánh giá từ 0-100 về khả năng trùng lặp ý tưởng với thư viện cũ (càng cao càng dễ trùng).`;
  ...
  try {
    ...
    console.error('AI Generation Error (Topics):', error);
    throw error; // Đã fix: Throw error ra ngoài để catch bên UI hoạt động
  }
};
...
export default function App() {
  ...
  // Lắng nghe trạng thái Gemini qua event
  useEffect(() => {
    const handleStatusUpdate = (e) => setGenerationStatus(e.detail);
    if (typeof window !== 'undefined') {
      window.addEventListener('haichai-gemini-status', handleStatusUpdate);
    }
    return () => {
      if (typeof window !== 'undefined') {
        window.removeEventListener('haichai-gemini-status', handleStatusUpdate);
      }
    };
  }, []);
  ...
  const handleTestGeminiKey = async () => {
    ...
    try {
      const result = await callGeminiWithRetry(
        ...
        1 // Chỉ test 1 lần, không cần retry
      );
  ...
  useEffect(() => {
    try {
      ...
      else setScripts(INITIAL_SCRIPTS);
  ...
  const saveScriptsToStorage = (newScripts) => setScripts(newScripts);
  const handleBibleChange = (e) => setBible(e.target.value);

  const handleSaveManualScript = () => {
    if (!manualScript.title.trim()) return showAlert('Lỗi', 'Vui lòng nhập tiêu đề kịch bản!');
    const newScript = {
      ...manualScript,
  ...
  const addManualScene = () => setManualScript({ ...manualScript, scenes: [...manualScript.scenes, { name: `Cảnh ${manualScript.scenes.length + 1}`, content: '', visualSuggestion: '' }] });
  const removeManualScene = (index) => setManualScript({ ...manualScript, scenes: manualScript.scenes.filter((_, i) => i !== index) });
  ...
  const handleScanLibrary = () => {
    ...
    setTimeout(() => {
      ...
      setScanResult({
        ...
        categories: counts,
        warning: scripts.length > 0 ? "Thư viện đã có nội dung. AI sẽ ưu tiên những chủ đề đang thiếu hụt." : 'Thư viện trống. An toàn để tạo mới.',
  ...
  const handleGenerateTopics = async () => {
    ...
    try {
      ...
      if (topics && topics.length > 0) {
        ...
        showAlert('Lỗi', 'AI không trả về dữ liệu. Có thể do lỗi mạng hoặc cấu hình Schema bị từ chối.');
      }
    } catch (error) {
  ...
  const toggleTopicSelection = (id) => setGeneratedTopics((topics) => topics.map((t) => (t.id === id ? { ...t, selected: !t.selected } : t)));
  ...
  const handleGenerateDetailedScripts = async () => {
    ...
    // Tối ưu: Dùng biến state cục bộ để show alert gộp lỗi (nếu có) vào cuối vòng lặp
    let errorMessages = [];
    ...
    for (let i = 0; i < selected.length; i += CHUNK_SIZE) {
      ...
      try {
        ...
        console.warn(`Lỗi tạo cụm kịch bản số ${chunkNumber}`, error);
        errorMessages.push(`Cụm ${chunkNumber}: ${error.message}`);
      }

      // Giảm delay xuống 2s do 2.5-flash tốc độ xử lý nhanh, ít khi bị dính rate limit dài
      if (i + CHUNK_SIZE < selected.length) {
        setGenerationStatus(`Hoàn thành cụm ${chunkNumber}. Nghỉ 2s để chống ngộp request...`);
    ...
    if (generatedCount === 0) {
      showAlert('Lỗi Tạo Kịch Bản', `Tất cả các lượt tạo đều thất bại.\nChi tiết: ${errorMessages.join('\n')}`);
    } else if (generatedCount < selected.length) {
      showAlert('Cảnh báo', `Đã tạo được ${generatedCount}/${selected.length} kịch bản. Một số bị bỏ qua do lỗi:\n${errorMessages.join('\n')}`);
    } else {
      showAlert('Thành công', `Đã tạo thành công toàn bộ ${generatedCount} kịch bản!`);
    }
  };
  ...
  const handleSaveDraftToLibrary = (draftId) => {
    ...
    if (draftToSave) {
      ...
      setDraftScripts((drafts) => drafts.filter((d) => d.id !== draftId));
      showAlert('Thành công', 'Đã lưu vào thư viện!');
    }
  };

  const handleDeleteScript = (id) => showConfirm('Xác nhận xóa', 'Bạn có chắc muốn xóa kịch bản này?', () => saveScriptsToStorage(scripts.filter((s) => s.id !== id)));
  const handleDeleteDraft = (id) => showConfirm('Xác nhận xóa', 'Bạn có chắc muốn xóa kịch bản nháp này?', () => setDraftScripts((drafts) => drafts.filter((d) => d.id !== id)));

  const handleExportPDF = (script) => {
    const printWindow = window.open('', '_blank');
    if (!printWindow) return showAlert('Lỗi Pop-up', 'Vui lòng cho phép trình duyệt mở tab mới (Pop-up) để in kịch bản.');
    const htmlContent = `
      <!DOCTYPE html>
      <html lang="vi">
      <head>
        ...
        <style>
          ...
          @media print { body { padding: 0; margin: 0; max-width: 100%; } }
        </style>
      </head>
      <body>
        <h1>${script.title}</h1>
        <div class="meta-box">
          <div class="meta-item"><span class="meta-label">Nhóm nội dung</span><span class="meta-value">${script.category}</span></div>
          <div class="meta-item"><span class="meta-label">Thông điệp chính</span><span class="meta-value">${script.mainMessage}</span></div>
        </div>
        <div class="hook-box">
        ...
        <div class="meta-item" style="margin-bottom: 16px;"><span class="meta-label">Text On Screen</span><span class="meta-value">${script.textOnScreen}</span></div>
        <div class="meta-item"><span class="meta-label">Caption</span><span class="meta-value" style="white-space: pre-wrap;">${script.caption}</span></div>
        ${script.notes ? `<h2 style="color: #b91c1c; border-bottom-color: #fecaca;">LƯU Ý AN TOÀN</h2><div class="danger-note">${script.notes}</div>` : ''}
    ...
    return (
      <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
        <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 animate-fade-in border border-slate-100">
          ...
          <p className="text-slate-600 mb-6 text-sm leading-relaxed whitespace-pre-line">{modal.message}</p>
          <div className="flex justify-end gap-3">
            {modal.type === 'confirm' && (
              <button onClick={closeModal} className="px-5 py-2 text-[#0d71ba] hover:bg-slate-100 rounded-lg font-medium transition">Hủy</button>
            )}
            <button onClick={() => { if (modal.onConfirm) modal.onConfirm(); closeModal(); }} className="px-5 py-2 bg-[#0d71ba] hover:opacity-90 text-white rounded-lg font-medium transition shadow-sm">
  ...
  const renderSidebar = () => (
    <div className="w-64 text-white flex flex-col h-screen fixed top-0 left-0 bg-[#0d2440]">
      ...
      <div className="p-4 text-xs text-slate-300 border-t border-slate-700">
        <div className="font-medium text-white/80">MVP v1.2 - Flash Optimized</div>
      ...
      <div className="animate-fade-in">
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6 flex gap-4">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Tìm tên kịch bản, chủ đề..." className="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
          <select className="border border-slate-300 rounded-lg px-4 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500" value={filterCat} onChange={(e) => setFilterCat(e.target.value)}>
              ...
              {filteredScripts.length === 0 ? (
                ...
                filteredScripts.map((script) => (
                  <tr key={script.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 text-slate-600"><span className="inline-block bg-slate-100 text-slate-700 px-2 py-1 rounded text-xs">{script.category}</span></td>
                    <td className="px-6 py-4"><span className={`px-2 py-1 rounded text-xs font-medium ${script.status === 'approved' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>{script.status.toUpperCase()}</span></td>
                    <td className="px-6 py-4"><span className={`px-2 py-1 rounded text-xs font-medium ${script.duplicateRiskScore > 70 ? 'bg-red-100 text-red-700' : script.duplicateRiskScore > 40 ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}`}>Score: {script.duplicateRiskScore}</span></td>
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden animate-fade-in">
              <div className="px-6 py-4 border-b border-slate-200 flex justify-between items-center bg-slate-50">
                ...
                <div className="flex gap-4 items-center">
                  <button onClick={() => handleExportPDF(viewingScript)} className="flex items-center gap-2 bg-slate-200 hover:bg-slate-300 text-slate-800 px-4 py-2 rounded-lg text-sm font-medium transition"><IconPrinter /> In kịch bản</button>
              ...
              <div className="p-6 overflow-y-auto grid grid-cols-1 lg:grid-cols-3 gap-8 text-sm">
                <div className="lg:col-span-2 space-y-6">
                  <div>
                    ...
                    <p className="bg-amber-50 border border-amber-200 text-amber-900 p-3 rounded-lg italic">{viewingScript.mainMessage}</p>
                  </div>
                  <div>
                ...
                <div className="space-y-6 bg-slate-50 p-4 rounded-lg border border-slate-100 h-fit">
                  <div><h4 className="font-bold text-slate-800 mb-1 uppercase text-xs">Text on Screen</h4><p className="text-slate-700 bg-white p-2 rounded border border-slate-200">{viewingScript.textOnScreen}</p></div>
                  <div><h4 className="font-bold text-slate-800 mb-1 uppercase text-xs">Caption</h4><p className="text-slate-700 bg-white p-2 rounded border border-slate-200 whitespace-pre-line">{viewingScript.caption}</p></div>
                  {viewingScript.notes && (<div><h4 className="font-bold text-red-800 mb-1 uppercase text-xs">Checklist An Toàn</h4><p className="text-red-700 bg-red-50 p-2 rounded border border-red-200">{viewingScript.notes}</p></div>)}
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="p-6 overflow-y-auto bg-white flex-1 space-y-6 text-sm">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="VD: Sai lầm khi chọn mặt bằng" value={manualScript.title} onChange={(e) => setManualScript({ ...manualScript, title: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Nhóm nội dung</label>
                    <select className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none" value={manualScript.category} onChange={(e) => setManualScript({ ...manualScript, category: e.target.value })}>
                      {CATEGORIES.map((c) => (<option key={c} value={c}>{c}</option>))}
                    </select>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="VD: Mặt bằng, Quản lý kho..." value={manualScript.topic} onChange={(e) => setManualScript({ ...manualScript, topic: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Thông điệp chính</label>
                    <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none" placeholder="Tóm tắt 1 câu thông điệp video" value={manualScript.mainMessage} onChange={(e) => setManualScript({ ...manualScript, mainMessage: e.target.value })} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea className="w-full px-3 py-2 border border-[#0d71ba] bg-blue-50/30 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none min-h-[60px]" placeholder="Câu nói đầu tiên thu hút người xem..." value={manualScript.selectedHook} onChange={(e) => setManualScript({ ...manualScript, selectedHook: e.target.value })} />
                </div>
                <div>
                  <div className="flex justify-between items-end mb-3">
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">Các phân cảnh chi tiết</label>
                    <button onClick={addManualScene} className="text-xs bg-[#0d71ba] text-white hover:opacity-90 px-3 py-1 rounded font-medium flex items-center gap-1 transition"><IconPlus /> Thêm cảnh</button>
                  </div>
                  <div className="space-y-4">
                    {manualScript.scenes.map((scene, index) => (
                      <div key={index} className="p-4 border border-slate-200 rounded-lg bg-slate-50 relative group">
                        <div className="flex justify-between mb-2">
                          <input type="text" className="font-bold bg-transparent border-none focus:ring-0 p-0 text-slate-700 text-sm w-32 outline-none" value={scene.name} onChange={(e) => updateManualScene(index, 'name', e.target.value)} placeholder="Tên cảnh..." />
                          {manualScript.scenes.length > 1 && (<button onClick={() => removeManualScene(index)} className="text-[#0d71ba] hover:opacity-70 opacity-0 group-hover:opacity-100 transition-opacity" title="Xóa cảnh"><IconTrash /></button>)}
                        </div>
                        <div className="space-y-3">
                          <textarea className="w-full px-3 py-2 border border-slate-300 rounded bg-white focus:ring-2 focus:ring-indigo-500 focus:outline-none min-h-[60px] text-sm" placeholder="Lời thoại (Voice / Nói trực tiếp)..." value={scene.content} onChange={(e) => updateManualScene(index, 'content', e.target.value)} />
                          <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded bg-white focus:ring-2 focus:ring-indigo-500 focus:outline-none text-sm text-slate-600 italic" placeholder="Góc máy / Hành động..." value={scene.visualSuggestion} onChange={(e) => updateManualScene(index, 'visualSuggestion', e.target.value)} />
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <textarea className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none min-h-[60px]" placeholder="Câu nói chốt lại vấn đề cuối video..." value={manualScript.ending} onChange={(e) => setManualScript({ ...manualScript, ending: e.target.value })} />
                </div>
                <hr className="border-slate-100" />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-4 rounded-lg border border-slate-100">
                  <div className="md:col-span-2">
                    ...
                    <input type="text" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none bg-white" placeholder="Tiêu đề to dán trên video..." value={manualScript.textOnScreen} onChange={(e) => setManualScript({ ...manualScript, textOnScreen: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Caption bài đăng (Kèm Hashtag)</label>
                    <textarea className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none min-h-[80px] bg-white" placeholder="Mô tả dưới bài đăng..." value={manualScript.caption} onChange={(e) => setManualScript({ ...manualScript, caption: e.target.value })} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-red-700 uppercase tracking-wider mb-2">Ghi chú an toàn (Nếu có)</label>
                    <textarea className="w-full px-3 py-2 border border-red-200 rounded-lg focus:ring-2 focus:ring-red-500 focus:outline-none min-h-[80px] bg-red-50" placeholder="Các từ ngữ cần tránh, lưu ý đạo cụ..." value={manualScript.notes} onChange={(e) => setManualScript({ ...manualScript, notes: e.target.value })} />
                  </div>
                </div>
              </div>
              <div className="px-6 py-4 border-t border-slate-200 flex justify-end gap-3 bg-slate-50">
                <button onClick={() => setIsAddingManual(false)} className="px-5 py-2 text-[#0d71ba] hover:bg-slate-200 rounded-lg font-medium transition">Hủy</button>
                <button onClick={handleSaveManualScript} className="px-5 py-2 bg-[#0d71ba] hover:opacity-90 text-white rounded-lg font-medium transition shadow-sm flex items-center gap-2"><IconCheck /> Lưu Kịch Bản</button>
              </div>
            </div>
          </div>
        )}
  ...
  const renderTopicGenerator = () => (
    <div className="animate-fade-in">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-8">
        ...
        <button onClick={handleScanLibrary} disabled={isScanning} className="bg-[#0d71ba] text-white px-5 py-2 rounded-lg font-medium transition disabled:opacity-50 flex items-center gap-2 hover:opacity-90 shadow-sm">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-sm text-slate-500">Sử dụng Gemini 2.5 Flash để tạo 30 chủ đề theo đúng Content Bible.</p>
          </div>
          <button onClick={handleGenerateTopics} disabled={isGeneratingTopics || !scanResult} className={`px-5 py-2 rounded-lg font-medium transition shadow-sm flex items-center gap-2 ${!scanResult ? 'bg-slate-300 text-slate-500 cursor-not-allowed' : 'bg-[#0d71ba] hover:opacity-90 text-white'}`}>
        ...
        {generatedTopics.length > 0 && (
          <div className="mt-6 border border-slate-200 rounded-xl overflow-hidden">
            <div className="bg-indigo-50 p-3 border-b border-slate-200 flex justify-between items-center text-sm">
              ...
              <button onClick={handleGenerateDetailedScripts} className="bg-[#0d71ba] text-white px-4 py-1.5 rounded hover:opacity-90 font-medium shadow-sm transition">Tạo kịch bản cho chủ đề đã chọn →</button>
            </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {generatedTopics.map((topic) => (
                    <tr key={topic.id} className={`hover:bg-indigo-50/70 cursor-pointer transition-colors ${topic.selected ? 'bg-indigo-50/50' : ''}`} onClick={() => toggleTopicSelection(topic.id)}>
                      <td className="px-4 py-4 text-center"><input type="checkbox" checked={topic.selected} onChange={() => {}} className="w-4 h-4 text-[#0d71ba] rounded border-slate-300 focus:ring-indigo-500" /></td>
                      <td className="px-4 py-4"><span className="inline-block bg-slate-100 text-slate-700 px-2 py-1 rounded text-xs font-medium">{topic.category}</span></td>
                      <td className="px-4 py-4"><p className="font-medium text-slate-900">{topic.topicName}</p><p className="text-slate-500 mt-1 line-clamp-2">{topic.angle}</p></td>
                      <td className="px-4 py-4"><span className={`px-2 py-1 rounded text-xs font-medium ${topic.duplicateRiskScore > 70 ? 'bg-red-100 text-red-700' : topic.duplicateRiskScore > 40 ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'}`}>{topic.duplicateRiskScore}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div key={script.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              <div className="bg-slate-50 px-6 py-4 border-b border-slate-200 flex justify-between items-center">
                ...
                <div className="flex gap-3">
                  <button onClick={() => handleSaveDraftToLibrary(script.id)} className="bg-[#0d71ba] hover:opacity-90 text-white px-4 py-2 rounded-lg font-medium text-sm flex items-center gap-2 transition shadow-sm"><IconCheck /> Lưu vào Thư viện</button>
                  <button onClick={() => handleDeleteDraft(script.id)} className="bg-[#0d71ba] hover:opacity-90 text-white px-3 py-2 rounded-lg font-medium text-sm flex items-center gap-2 transition shadow-sm" title="Xóa nháp này"><IconTrash /> Xóa</button>
                </div>
              </div>
              <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2 space-y-6 text-sm">
                  <div><h4 className="font-bold text-slate-800 mb-2 uppercase text-xs tracking-wider">Thông điệp chính</h4><p className="bg-amber-50 border border-amber-200 text-amber-900 p-3 rounded-lg italic">{script.mainMessage}</p></div>
                  <div>
                    <h4 className="font-bold text-slate-800 mb-2 uppercase text-xs tracking-wider">Hook Được Chọn</h4>
                    <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-lg">
                  ...
                  <div><h4 className="font-bold text-slate-800 mb-2 uppercase text-xs tracking-wider">Câu Kết</h4><p className="text-slate-800 font-medium">"{script.ending}"</p></div>
                </div>
                <div className="space-y-6 text-sm bg-slate-50 p-4 rounded-lg border border-slate-100 h-fit">
                  <div><h4 className="font-bold text-slate-800 mb-1 uppercase text-xs">Text on Screen</h4><p className="text-slate-700 bg-white p-2 rounded border border-slate-200">{script.textOnScreen}</p></div>
                  <div><h4 className="font-bold text-slate-800 mb-1 uppercase text-xs">Caption</h4><p className="text-slate-700 bg-white p-2 rounded border border-slate-200 whitespace-pre-line">{script.caption}</p></div>
                  {script.notes && (<div><h4 className="font-bold text-red-800 mb-1 uppercase text-xs">Checklist An Toàn</h4><p className="text-red-700 bg-red-50 p-2 rounded border border-red-200">{script.notes}</p></div>)}
                </div>
              </div>
            </div>
          ))}
  ...
  const renderBible = () => (
    <div className="animate-fade-in flex flex-col h-[calc(100vh-6rem)]">
      ...
      <textarea className="flex-1 w-full p-6 border border-slate-300 rounded-xl shadow-inner focus:ring-2 focus:ring-indigo-500 focus:outline-none font-mono text-sm resize-none bg-slate-50 text-slate-800 leading-relaxed" value={bible} onChange={handleBibleChange} spellCheck="false" />
    </div>
  );

  const renderSettings = () => {
    const handleExport = () => {
      const backup = { bible, scripts, draftScripts, generatedTopics, exportedAt: new Date().toISOString(), ownerEmail: currentUser?.email || '' };
      ...
      <div className="animate-fade-in max-w-3xl">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-6">
          ...
          <div className="flex gap-3 mt-5">
            {currentUser ? (<button onClick={handleSignOut} className="bg-[#0d71ba] text-white px-4 py-2 rounded-lg font-medium text-sm transition hover:opacity-90 shadow-sm">Đăng xuất</button>) : (<button onClick={handleSignInWithGoogle} className="bg-[#0d71ba] text-white px-4 py-2 rounded-lg font-medium text-sm transition hover:opacity-90 shadow-sm">Đăng nhập Google công ty</button>)}
            <button onClick={handleExport} className="bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 px-4 py-2 rounded-lg font-medium text-sm">Export Backup JSON</button>
          </div>
          {authError && (<div className="mt-4 p-3 rounded-lg bg-red-50 border border-red-200 text-red-700 text-sm">{authError}</div>)}
        </div>

        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-6">
          <h3 className="text-lg font-bold mb-4">Gemini API Key</h3>
          <p className="text-sm text-slate-500 mb-4">Key này dùng để gọi Google Gemini (Model: 2.5 Flash). App lưu key trên trình duyệt của máy đang dùng, không đẩy key lên Firestore.</p>
          <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Nhập Gemini API Key</label>
          <input type="password" className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none font-mono text-sm" placeholder="AIza..." value={geminiApiKey} onChange={(e) => { setGeminiApiKey(e.target.value); setGeminiKeyStatus(''); }} />
          <div className="flex flex-wrap gap-3 mt-4">
            <button onClick={handleSaveGeminiKey} className="bg-[#0d71ba] text-white px-4 py-2 rounded-lg font-medium text-sm transition hover:opacity-90 shadow-sm">Lưu Gemini API Key</button>
            <button onClick={handleTestGeminiKey} disabled={isTestingGeminiKey} className="bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 px-4 py-2 rounded-lg font-medium text-sm disabled:opacity-50">{isTestingGeminiKey ? 'Đang test...' : 'Test Gemini Key'}</button>
            <button onClick={() => { setGeminiApiKey(''); localStorage.removeItem('gemini_api_key'); setGeminiKeyStatus('Đã xoá Gemini API Key trên máy này.'); }} className="bg-white border border-red-200 text-red-700 hover:bg-red-50 px-4 py-2 rounded-lg font-medium text-sm">Xoá Key</button>
          </div>
          {geminiKeyStatus && (<div className={`mt-4 p-3 rounded-lg text-sm border ${geminiKeyStatus.includes('được') || geminiKeyStatus.includes('Đã lưu') ? 'bg-green-50 border-green-200 text-green-700' : 'bg-amber-50 border-amber-200 text-amber-700'}`}>{geminiKeyStatus}</div>)}
        </div>
      </div>
    );
  };
  ...
  const renderLoginScreen = () => (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl border border-slate-200 p-8">
        <div className="mb-6">
          <img src="https://tascusfood.com/haichailogo.png" onError={(e) => { e.target.onerror = null; e.target.src = 'https://placehold.co/180x60/0d2440/FFF?text=HAICHAI+STUDIO'; }} alt="Haichai Script Studio" className="h-16 w-auto object-contain mb-4" />
          <h1 className="text-2xl font-bold text-slate-900">Haichai Script Studio</h1>
          <p className="text-sm text-slate-500 mt-2">Đăng nhập bằng Google Workspace công ty để sử dụng thư viện kịch bản và đồng bộ Firestore.</p>
        </div>
        ...
        {authError && (<div className="rounded-lg bg-red-50 border border-red-200 p-4 text-sm text-red-700 mb-5">{authError}</div>)}
        <button onClick={handleSignInWithGoogle} className="w-full bg-[#0d71ba] hover:opacity-90 text-white px-5 py-3 rounded-lg font-semibold transition shadow-sm">Đăng nhập bằng Google công ty</button>
      </div>
      {renderModal()}
    </div>
  );

  if (authLoading) return (<div className="min-h-screen bg-slate-50 flex items-center justify-center font-sans text-slate-600">Đang kiểm tra đăng nhập Firebase...</div>);
