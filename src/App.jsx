...
const fetchWithTimeout = async (
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      throw new Error(
        `Gemini không phản hồi sau ${Math.round(
          timeoutMs / 1000
        )} giây. Hãy bấm “Test Gemini Key” ở Cài đặt & Dữ liệu để kiểm tra key/model trước.`
      );
    }
    throw error;
  } finally {
...
const shouldTryNextModel = (status, message = '') => {
  ...
  return (
    status === 404 ||
    (status === 400 && (m.includes('model') || m.includes('not found')))
  );
};
...
const callGeminiWithRetry = async (
  ...
  if (!apiKey) {
    throw new Error(
      'Bạn chưa nhập Gemini API Key. Vào “Cài đặt & Dữ liệu” để nhập key trước.'
    );
  }
  ...
  for (const model of GEMINI_MODELS) {
    for (let i = 0; i < retries; i++) {
      try {
        emitGeminiStatus(
          `Đang gọi Gemini: ${model}${
            retries > 1 ? ` (${i + 1}/${retries})` : ''
          }...`
        );
        ...
        if (!response.ok) {
          ...
          const err = new Error(
            normalizeGeminiError(response.status, rawMessage)
          );
  ...
  const handleTestGeminiKey = async () => {
    ...
    try {
      ...
      if (result?.ok) {
        ...
        setGeminiKeyStatus(
          'Gemini có phản hồi nhưng JSON không đúng format mong đợi.'
        );
      }
    } catch (error) {
  ...
  const handleScanLibrary = () => {
    ...
    setTimeout(() => {
      ...
      setScanResult({
        ...
        warning:
          scripts.length > 0
            ? "Thư viện đã có nội dung. Chú ý các chủ đề về 'Mặt bằng' đang lặp lại."
            : 'Thư viện trống. An toàn để tạo mới.',
        suggestedFocus:
          "Nên tập trung vào nhóm 'Sản phẩm / niềm tin / giấy tờ' vì đang thiếu.",
      });
      setIsScanning(false);
    }, 800);
  };
  ...
  const handleGenerateTopics = async () => {
    ...
    if (!savedKey) {
      ...
      return showAlert(
        ...
        'Vào Cài đặt & Dữ liệu, dán Gemini API Key rồi bấm “Lưu Gemini API Key” trước khi tạo chủ đề.'
      );
    }
  ...
  const handleGenerateDetailedScripts = async () => {
    ...
    for (let i = 0; i < selected.length; i += CHUNK_SIZE) {
      ...
      if (i + CHUNK_SIZE < selected.length) {
        setGenerationStatus(
          `Hoàn thành cụm ${chunkNumber}. Nghỉ 5s để tránh nghẽn mạng...`
        );
    ...
    if (generatedCount === 0) {
      ...
      showAlert(
        'Thông báo',
        `Đã tạo thành công ${generatedCount}/${selected.length} kịch bản. Một số bị bỏ qua do giới hạn của AI.`
      );
    } else {
    ...
    <div className="animate-fade-in">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-8">
        ...
        <p className="text-sm text-slate-500 mb-4">
          AI sẽ phân tích các kịch bản đã có để tránh tạo trùng ý tưởng, trùng
          hook.
        </p>
        <button
        ...
        {scanResult && (
          <div className="mt-4 p-4 bg-slate-50 rounded-lg text-sm text-slate-700 border border-slate-200">
            <p>
              <strong>Tổng số kịch bản:</strong> {scanResult.total}
            </p>
            <p>
              <strong>Cảnh báo AI:</strong>{' '}
              <span className="text-amber-600">{scanResult.warning}</span>
            </p>
            <p>
              <strong>Gợi ý:</strong>{' '}
              <span className="text-indigo-600">
                {scanResult.suggestedFocus}
              </span>
            </p>
          </div>
        )}
    ...
    return (
      <div className="animate-fade-in max-w-3xl">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 mb-6">
          ...
          <p className="text-sm text-slate-500 mb-4">
            Key này dùng để gọi Google Gemini khi tạo chủ đề/kịch bản. App lưu
            key trên trình duyệt của máy đang dùng, không đẩy key lên Firestore.
          </p>
          ...
          {geminiKeyStatus && (
            <div
              className={`mt-4 p-3 rounded-lg text-sm border ${
                geminiKeyStatus.includes('được') ||
                geminiKeyStatus.includes('Đã lưu')
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200">
          ...
          <div className="text-sm text-slate-600 space-y-2">
            ...
            <p>
              • <strong>Gemini API Key:</strong> chỉ lưu trên máy người dùng,
